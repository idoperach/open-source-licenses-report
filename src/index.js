...
const recursiveFilesScan = async (path) => {
  ...
  const dirs = dirents
    .filter((direnet) => direnet.isDirectory())
    .map((direnet) => direnet.name);
...
const licenseScan = async (path) => {
  ...
  cmd.runSync(
    `license-checker --start ${path} --csv --out ${join(
      OUTPATH,
      `${csvFileName}.csv`
    )}`
  );
...
const buildExcel = async () => {
  ...
  const files = dirents.filter(
    (direnet) => !direnet.isDirectory() && direnet.name.endsWith("csv")
  );
  const CSVFiles = [];
